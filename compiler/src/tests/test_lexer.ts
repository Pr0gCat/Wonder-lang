...
describe('Lexer', () => {
    function checkToken(token: Token | undefined, expect_type: string, expect_value: string){
        assert.equal(token?.type, expect_type, `Lexer should returns ${expect_type} token not ${token?.type}.`);
        assert.equal(token?.value, expect_value, `Attribute value expected to be ${expect_value} not ${token?.value}.`);
    }
